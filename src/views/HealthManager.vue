<template>
  <div class="healthmanager">
    ...
    <van-cell-group v-for="(item) of usergroup" :key="item.id">
      ...
      <van-field v-else-if="item.status == 1" value="已激活" label="设备状态 : " />
      <van-field v-else-if="item.status == 2" value="已挂失" label="设备状态 : " />
      <van-field v-else-if="item.status == 3" value="已解绑" label="设备状态 : " />
